...
- **Research Cortex:** Backend engine (`server/research-cortex.ts`) at `/cortex` that auto-indexes all docs in `docs/context-docs/` (currently 19), extracting 1400+ claims by category (constant/entity/mechanism/evidence/prediction/connection/definition). Handles structured data (PubMed TSV tables, DOI links) and free-form research. Features: 12-facet dodecahedral synthesis engine dispatching to model-optimized angles (reasoning→o3-mini, critique→Claude, tonal/semantic→Qwen/DeepSeek, math→deepseek-prover, geopolitical→Claude, synthesis→Gemini 2.5 Pro), contradiction detection, knowledge gap analysis, domain mapping, and multi-format export (Markdown/JSON/LaTeX). UI has 5 tabs: Corpus, Claims, Analysis, Synthesis, Reader.
