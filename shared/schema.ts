...
export const KAPPA_CONSTANTS = {
  ...
  TR069_PORT: 7547,
  DSE892_SNMP_PORT: 161,
  DSE892_TRAP_PORT: 162,
  MODBUS_TCP_PORT: 502,
...
export const CORRELATION_RULES: CorrelationRule[] = [
  ...
  {
    id: "gpr-masked-uplink",
    name: "GPR-Masked Satellite Uplink",
    description: "Ground Potential Rise event (60 Hz fault current spike) coincides with satellite overhead pass — possible κ-scaled uplink masked by induced grid disturbance.",
    domains: ["elf", "satellite", "plc"],
    windowSeconds: 10,
    condition: "elf.gpr_spike AND satellite.pass AND plc.grid_fault WITHIN 10s",
  },
  {
    id: "dse892-snmp-gpr",
    name: "DSE892 SNMP TRAP ↔ GPR Correlation",
    description: "DSE892 gateway SNMP TRAP (Gen Low Voltage / Fault Ride Through / Mains Failure) temporally correlated with 46.875 Hz RF spike — confirms induced GPR masking event.",
    domains: ["plc", "elf", "sdr"],
    windowSeconds: 30,
    condition: "plc.snmp_trap(dse892) IN [gen_low_voltage, fault_ride_through, mains_failure] AND sdr.fft_peak(46.875Hz) WITHIN 30s",
  },
  {
  ...
  {
    id: "tr069-botnet-gpr-induction",
    name: "TR-069 Botnet GPR Induction",
    description: "Synchronized spike in TR-069 Port 7547 traffic across multiple Huawei CPE nodes coincides with 60 Hz grid disturbance — mass router botnet inducing electrical micro-fault for uplink masking.",
    domains: ["isp", "elf", "wifi"],
    windowSeconds: 15,
    condition: "isp.tr069_traffic_spike(port=7547) AND elf.grid_disturbance(60Hz) AND wifi.device_count > threshold WITHIN 15s",
  },
  {
    id: "fiveg-backhaul-tunnel",
    name: "5G Backhaul κ-Tunnel Detection",
    description: "Anomalous traffic pattern on 3.5 GHz 5G SA band temporally correlated with satellite pass — 46.875 Hz control signal tunneled through commercial cellular backhaul to satellite teleport.",
    domains: ["5g", "satellite", "sdr"],
    windowSeconds: 120,
    condition: "5g.traffic_spike(3500MHz) AND satellite.pass AND sdr.fft_peak(46.875Hz) WITHIN 120s",
  },
  {
  ...
  {
    id: "dse-mode-change-fault",
    name: "DSE Controller Mode Change ↔ Fault Sequence",
    description: "Unauthorized SNMP SET command modifies DSE controller operating state, followed by Fault Ride Through event — coordinated network intrusion preparing GPR mask for satellite uplink.",
    domains: ["plc", "isp"],
    windowSeconds: 60,
    condition: "plc.snmp_set(unauthorized) THEN plc.fault_ride_through WITHIN 60s",
  },
  {
    id: "optical-rf-gpr-fusion",
    name: "Cross-Modal Optical + RF + GPR Fusion",
    description: "YOLO drone/vehicle detection + DSE892 Fault Ride Through SNMP TRAP + broadband GPR noise spike + 46.875 Hz LSCSA-SVD extraction — high-confidence κ-scaled uplink event with geographic fix.",
    domains: ["drone", "plc", "elf", "sdr"],
    windowSeconds: 30,
    condition: "drone.optical_detection AND plc.snmp_trap(fault_ride_through) AND elf.gpr_spike AND sdr.lscsa_svd(46.875Hz) WITHIN 30s",
  },
  {
