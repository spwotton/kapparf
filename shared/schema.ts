...
export const CORRELATION_RULES: CorrelationRule[] = [
  ...
  {
    id: "sdr-satellite-temporal",
    name: "SDR Detection ↔ Satellite Pass",
    description: "KiwiSDR detected signal energy at a monitored frequency during a satellite overhead pass — temporal coincidence logged for review.",
    domains: ["sdr", "satellite"],
    windowSeconds: 120,
    condition: "sdr.snr_detection AND satellite.elevation > 30° WITHIN 120s",
  },
  {
    id: "vlf-snr-detection",
    name: "VLF SNR Detection",
    description: "KiwiSDR measured above-threshold SNR at a VLF harmonic frequency (46.875/93.75/140.625/187.5 Hz range via KiwiSDR passband).",
    domains: ["sdr"],
    windowSeconds: 30,
    condition: "sdr.snr > threshold AT vlf_target_freq",
  },
  {
    id: "morse-cw-detection",
    name: "Morse/CW Signal Detection",
    description: "KiwiSDR detected dit/dah keying pattern at a monitored frequency — Morse code beacon or CW transmission identified with WPM estimate.",
    domains: ["morse"],
    windowSeconds: 60,
    condition: "morse.keying_pattern_detected AND morse.wpm > 0",
  },
  {
    id: "blackjack-mandrake-hf",
    name: "BLACKJACK HF Frequency Detection",
    description: "KiwiSDR detected signal energy at 2274 kHz (BLACKJACK MANDRAKE coordination frequency) or its harmonics (4548/6822/9096 kHz). Passive HF monitoring only.",
    domains: ["sdr"],
    windowSeconds: 60,
    condition: "sdr.detection AT blackjack_mandrake_freq",
  },
  {
    id: "weather-satellite-correlation",
    name: "Weather Conditions ↔ Satellite Pass",
    description: "Atmospheric conditions logged during satellite overhead pass — relevant for signal propagation analysis.",
    domains: ["elf", "satellite"],
    windowSeconds: 300,
    condition: "elf.atmospheric_conditions AND satellite.pass WITHIN 300s",
  },
  {
    id: "flight-satellite-coincidence",
    name: "Flight ↔ Satellite Temporal Coincidence",
    description: "Aircraft detected via ADS-B data during satellite overhead pass — logged for airspace activity correlation.",
    domains: ["radar", "satellite"],
    windowSeconds: 120,
    condition: "radar.flight_detected AND satellite.pass WITHIN 120s",
  },
];
...
export const KARACHI_MODULES: KarachiModule[] = [
  ...
  {
    id: "kiwisdr-passive-monitor",
    name: "KIWISDR-MONITOR",
    codename: "HF Passive Scanner",
    base: "KiwiSDR WebSocket API",
    purpose: "Passive HF spectrum monitoring via remote KiwiSDR nodes",
    category: "flow-analysis",
    domains: ["sdr"],
    implementation: "Node.js WebSocket client connecting to KiwiSDR nodes for SNR readings at target frequencies",
    capabilities: ["SNR measurement at target frequencies", "Morse/CW keying detection", "BART burst pattern analysis"],
    useCase: "Monitor HF spectrum for signal activity at VLF harmonics, Riemann zero frequencies, and BLACKJACK coordination frequencies",
    target: "KiwiSDR nodes (TI0RC, Puntarenas, PJ4G Bonaire)",
    vulnerability: "N/A — passive monitoring only",
  },
  {
    id: "celestrak-tracker",
    name: "CELESTRAK-TRACKER",
    codename: "Satellite Pass Tracker",
    base: "CelesTrak TLE API + satellite.js",
    purpose: "Track satellite overhead passes from observer location",
    category: "flow-analysis",
    domains: ["satellite"],
    implementation: "Fetches TLE data from CelesTrak, computes real-time elevation/azimuth/range using satellite.js SGP4 propagator",
    capabilities: ["Overhead pass detection", "Elevation/azimuth computation", "Multi-constellation tracking"],
    useCase: "Log satellite passes above observer for temporal correlation with other domain events",
    target: "CelesTrak active satellite catalog",
    vulnerability: "N/A — passive monitoring only",
  },
];
...
export const CONGUSTO_MODULES: CongustoModule[] = [
  ...
  },
  {
    ...
    description: "Matches events across domains using time-window rules — only fires when all required domains have active events",
    technology: "Node.js",
    implementation: "Scans recent events (last 5 minutes) against correlation rules. Each rule specifies required domains and a time window. Only matches when ALL specified domains have events within the window. Deduplicates by event pair.",
    features: ["Multi-domain time-window matching", "Strict domain requirement (all must be present)", "Event pair deduplication", "Configurable window sizes"],
  },
];
