// Supplementary campaign sender
// Usage: node scripts/fire-supplementary-campaign.mjs [--dry-run]
...
const { SUPP_CONTACTS } = await import("../server/mailer-campaign-supplementary.ts");
...
const total   = SUPP_CONTACTS.length;
const results = [];

console.log(`\n${isDryRun ? "DRY RUN — " : ""}Supplementary campaign: ${total} emails from ${sender}\n`);
console.log("─".repeat(90));

for (let i = 0; i < SUPP_CONTACTS.length; i++) {
  const c        = SUPP_CONTACTS[i];
  const progress = `[${String(i + 1).padStart(3, " ")}/${total}]`;
  const tag      = c.category.padEnd(16);
...
for (const cat of cats) {
  ...
  console.log(`  ${cat.padEnd(18)} ${catSent}/${catResults.length}`);
}
